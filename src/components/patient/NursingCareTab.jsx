...
export default function NursingCareTab({ patientId, canEdit }) {
  ...
  const patientCarePlans = nursingCare.filter(c => c.patientId === patientId && c.recurring);
  ...
  const handleSubmit = (e) => {
    ...
    addNursingCare({ patientId, ...formData, createdBy: displayName, createdAt: new Date().toISOString() });
  ...
  const handlePeriodClick = (careId, period) => {
    updateNursingCarePeriod(careId, today, period);
    toast({ title: "Período confirmado!", description: `Cuidado do período da ${period === 'morning' ? 'manhã' : period} realizado.` });
  };

  const PeriodButton = ({ care, period, icon: Icon, label }) => {
    const isCompleted = care.dailyRecords?.[today]?.[period]?.completed;
    return (
      <Button
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <Card>
          ...
          <CardContent className="space-y-4">
            {patientCarePlans.length > 0 ? patientCarePlans.map(care => (
              <div key={care.id} className="p-3 rounded-lg bg-muted/50">
                ...
                {Object.entries(care.dailyRecords?.[today] || {}).map(([period, data]) => (
                  data.completed && <p key={period} className="text-green-600 text-xs mt-1 capitalize">{period === 'morning' ? 'Manhã' : period === 'afternoon' ? 'Tarde' : 'Noite'}: {new Date(data.completedAt).toLocaleTimeString('pt-BR')}</p>
                ))}
              </div>
            )) : <p className="text-muted-foreground text-center py-4">Nenhum plano de cuidado recorrente para este paciente.</p>}
          </CardContent>
        </Card>
      </motion.div>
    </div>
