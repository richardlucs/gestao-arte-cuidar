...
export default function VitalSignsTab({ residentId, residentName, canEdit }) {
  ...
  const residentVitalSigns = vitalSigns.filter(v => v.residentId === residentId);
  const today = new Date().toISOString().split('T')[0];
  const todaysSign = residentVitalSigns.find(v => v.date === today);
  ...
  const handleSubmit = (e) => {
    ...
    const signs = {
      residentId, caregiverName: user?.name,
    ...
    if (alerts.length > 0) {
      ...
      setIsAlertOpen(true);
      alerts.forEach(msg => addAlert({ type: 'vital', message: msg, residentId }));
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {residentVitalSigns.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map((s, i) => (
