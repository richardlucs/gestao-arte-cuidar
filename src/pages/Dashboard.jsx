...
export default function Dashboard() {
  ...
  const menuItems = [
    ...
    { title: 'Gerenciar Residentes', description: 'Cadastrar e editar dados de residentes', icon: UserPlus, path: '/gerenciar-residentes', color: 'bg-green-500', available: ['supervisor', 'administrador'].includes(user?.role) },
    { title: 'Gerenciar Usuários', description: 'Adicionar e editar usuários do sistema', icon: Users2, path: '/gerenciar-usuarios', color: 'bg-blue-500', available: ['supervisor', 'administrador'].includes(user?.role) },
    { title: 'Estoque de Medicação', description: 'Controlar estoque individual dos residentes', icon: Package, path: '/residentes?openStock=true', color: 'bg-yellow-500', available: ['supervisor', 'administrador'].includes(user?.role) }
  ];

  const features = [
    { title: 'Prescrição Médica', description: 'Controle de medicamentos', icon: FileText, color: 'bg-purple-500', tab: 'prescription', available: user?.role !== 'terceirizado' },
    { title: 'Cuidados de Enfermagem', description: 'Procedimentos especializados', icon: Heart, color: 'bg-pink-500', tab: 'nursing', available: user?.role !== 'terceirizado' },
    { title: 'Controle Básico', description: 'Evacuação, banho e alimentação', icon: Users, color: 'bg-orange-500', tab: 'basic', available: user?.role !== 'terceirizado' },
    { title: 'Sinais Vitais', description: 'Pressão, frequência, etc.', icon: Activity, color: 'bg-destructive', tab: 'vitals', available: user?.role !== 'terceirizado' },
