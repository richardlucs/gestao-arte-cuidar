...
export default function UserManagement() {
  const { user: loggedInUser, displayName } = useAuth();
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2"><Label htmlFor="role">Função *</Label>
                          <Select value={formData.role} onValueChange={(value) => setFormData({...formData, role: value})}>
                            ...
                            <SelectContent>
                              {loggedInUserRole === 'administrador' && <SelectItem value="administrador">Administrador</SelectItem>}
                              {(loggedInUserRole === 'administrador' || loggedInUserRole === 'supervisor') && <SelectItem value="supervisor">Supervisor</SelectItem>}
            ...
            <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {users && users.map(user => (
                        <motion.div key={user.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                        <Card>
                            <CardHeader>
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center space-x-3">
                                        <div className={`p-3 rounded-full ${getRoleColor(user.role)}`}>{getRoleIcon(user.role)}</div>
                                        <div>
                                    ...
                                    <div className="flex gap-1">
                                        ...
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                                ...
                                                <AlertDialogAction onClick={() => handleDeleteClick(user.id)} className="bg-destructive hover:bg-destructive/90">Remover</AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                                        </AlertDialog>
                                    </div>
